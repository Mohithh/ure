...
export default function SiteHeader() {
  ...
  // ✅ KEY FIX: Close menu whenever the pathname changes (user navigated)
  useEffect(() => {
    setMenuOpen(false);
  }, [pathname]);

  // ✅ KEY FIX: Prevent body scroll when menu is open
  useEffect(() => {
    if (menuOpen) {
  ...
  const hideHeaderIcons = suppressedPrefixes.some((prefix) => {
  ...
  return (
    <>
        ...
        <div className="max-w-6xl w-full mx-auto py-7 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4 lg:gap-8 shrink-0">
            ...
            <button
              ...
              className="flex flex-col justify-between w-8 h-5 cursor-pointer shrink-0 gap-1.5"
            ...
            >
              <div
                className="w-9 h-9 rounded-full bg-[#C15F3C] border-2 border-cloudy/30 flex items-center justify-center text-[#F4F3EE] text-xs font-bold transition-transform group-hover:scale-105"
              ...
              <div className="flex flex-col">
                <span
                  className="text-[#C15F3C] text-[14px] font-bold uppercase tracking-tight leading-none"
            ...
            <ul className="flex items-center gap-3 list-none m-0 p-0">
              {navItems.map((item) => (
                <li key={item.label} className="relative group">
                  ...
                  {/* Dropdown */}
                  {item.dropdown && (
                    <div className="absolute top-full left-0 bg-[#F4F3EE] border border-cloudy/30 min-w-[220px] shadow-md opacity-0 invisible translate-y-2 group-hover:opacity-100 group-hover:visible group-hover:translate-y-0 transition-all duration-300 py-3 z-50">
                      <ul className="list-none m-0 p-0">
                        {item.dropdown.map((sub) => (
                          <li key={sub.label}>
                            <Link
                              href={sub.href}
                              className="block px-6 py-2.5 text-[#C15F3C] text-[12px] capitalize hover:scale-105 hover:bg-[#B1ADA1]/30 transition-all duration-200 whitespace-nowrap"
                              style={{ fontFamily: "'Open Sans', sans-serif" }}
                            >
                              {sub.label}
                            </Link>
            ...
            {!hideHeaderIcons && (
              <div className="hidden lg:flex items-center gap-2">
                ...
                <Link
                  ...
                  className="text-[#C15F3C] p-2 hover:scale-110 transition-all duration-300"
                  aria-label="LinkedIn"
                >
      ...
      {/* ✅ KEY FIX: Pass closeMenu handler so FullscreenMenu can close itself on link click */}
      <FullscreenMenu
        isOpen={menuOpen}
